...
if (!empty($_GET['request_id'])) {
    ...
    $stmt = $conn->prepare("SELECT name,status,reason,shift_type,request_type,created_at FROM student_transfers WHERE id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        <body>
            <div class="container">
                <h1>تفاصيل الطلب</h1>
                <p class="info"><strong>اسم الطالب:</strong> <?php echo htmlspecialchars($row['name']); ?></p>
                <p class="info"><strong>نوع الطلب :</strong> <?php echo htmlspecialchars($row['request_type']); ?></p>
                <p class="info"><strong>تحويل الدراسه من : </strong> <?php echo htmlspecialchars($row['shift_type']); ?></p>

                <p class="info"><strong>السبب :</strong> <?php echo htmlspecialchars($row['reason']); ?></p>
